...
/*
 一時ログファイル
 破棄可能なテキストファイルにテキストメッセージを書き出すロギングクラスを作り
 なさい。テキストファイルは、一意の識別名を持ち、一時ディレクトリに配置します。
 特に指定しない限り、クラスインスタンスが削除されるときに、このログファイルは破
 棄されます。しかし、ログファイルを永続的な位置に移して保持することもできるよう
 にします。
 */
...
int main()
{
  // C++のコンストラクタについて分かっていないかも
  // https://teratail.com/questions/51082
   logger log;
   try
   {
